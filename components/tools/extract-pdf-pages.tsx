...
import {
  UploadCloud, FileText, Trash2, Loader2, ArrowDownToLine
} from "lucide-react";
...
import { extractPdfPages } from "@/lib/pdf-engine";

const FAQS: FAQItem[] = [
  { question: "How do I specify pages to extract?", answer: "Enter a comma-separated list of pages or ranges. For example: 1, 3, 5-7. These pages will be pulled into a new PDF document." },
  ...
  { question: "Can I extract pages from a password-protected PDF?", answer: "No, you must unlock the PDF first before you can extract pages from it." },
];
...
export function ExtractPdfPagesTool() {
  ...
  const handleProcess = async () => {
    ...
    try {
      const pagesToExtract = parsePageRanges(pagesInput);
      const processed = await extractPdfPages(file, pagesToExtract);
      const blob = new Blob([new Uint8Array(processed) as BlobPart], { type: "application/pdf" });
      const url = URL.createObjectURL(blob);
      const newName = file.name.replace(/\.pdf$/i, "-extracted.pdf");
  ...
  return (
    <ToolPageLayout
      ...
      description="Pull out specific pages from your PDF to create a new document securely."
    >
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <ToolSectionCard title="Select Document" subtitle="Upload the PDF you want to extract pages from">
            {!file ? (
              <div
              ...
              <div className="mt-6 space-y-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-2">
                    Enter the exact page numbers you want to extract into the new document.
                  </p>
                </div>
        ...
        <div className="lg:col-span-1">
          <FAQSection toolName="Extract PDF Pages" items={FAQS} />
        </div>
      </div>

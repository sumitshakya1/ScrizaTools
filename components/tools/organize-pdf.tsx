...
import {
  UploadCloud, FileText, Trash2, Loader2, Layers
} from "lucide-react";
...
import { organizePdfPages } from "@/lib/pdf-engine";

const FAQS: FAQItem[] = [
  { question: "How do I reorder pages?", answer: "Simply type the page numbers in the order you want them to appear, separated by commas (e.g. 3, 1, 2)." },
  { question: "Can I duplicate or delete pages this way?", answer: "Yes! If you type '1, 1, 2', page 1 will appear twice. If you omit a page number, it will be removed." },
...
export function OrganizePdfTool() {
  ...
  const handleProcess = async () => {
    if (!file || !orderInput.trim()) return;
    setIsProcessing(true);
    try {
      const newOrder = parsePageOrder(orderInput);
      const processed = await organizePdfPages(file, newOrder);
      const blob = new Blob([new Uint8Array(processed) as BlobPart], { type: "application/pdf" });
      const url = URL.createObjectURL(blob);
      const newName = file.name.replace(/\.pdf$/i, "-organized.pdf");
      setPendingDownload({ url, name: newName });
      setShowCountdown(true);
    } catch (error) {
      console.error("Failed to organize pages:", error);
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <ToolSectionCard title="Select Document" subtitle="Upload the PDF you want to organize">
            {!file ? (
              <div
              ...
              <div className="mt-6 space-y-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-2">
                    List the page numbers in the exact order you want them to appear.
                  </p>
                </div>
        ...
        <div className="lg:col-span-1">
          <FAQSection toolName="Organize PDF" items={FAQS} />
        </div>
      </div>

...
import { convertWordToPdf } from "@/lib/pdf-engine";
...
export function WordToPdfTool() {
  ...
  return (
    <ToolPageLayout toolId="word-to-pdf" title="Word to PDF" description="Convert Microsoft Word (.doc, .docx) documents to PDF format.">
      <div className="space-y-6">
        <ToolSectionCard title="Upload Word Document" subtitle="Select a .doc or .docx file to convert to PDF.">
          <div
          ...
          >
            ...
            <p className="text-sm font-semibold text-on-surface">Drop Word file here or click to browse</p>
            <p className="text-xs text-tertiary mt-1">Supports .doc, .docx files</p>
        ...
        <FAQSection toolName="Word to PDF" items={[
          { question: "What Word formats are supported?", answer: "Both .doc and .docx (Microsoft Word) file formats are supported for conversion." },
          { question: "Is the conversion done on a server?", answer: "No. All conversion happens 100% in your browser. Your files never leave your device." },
          { question: "Are images preserved?", answer: "Embedded images and basic formatting are preserved during the conversion process." },
        ]} />
      </div>

      <DownloadCountdownModal isOpen={showCountdown} fileName={pendingDownload?.name || "document.pdf"} onComplete={triggerDownload} onClose={() => setShowCountdown(false)} />
    </ToolPageLayout>
  );
}
